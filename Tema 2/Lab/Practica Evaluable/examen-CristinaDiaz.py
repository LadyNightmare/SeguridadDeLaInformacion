...
# Ejercicio
# ----------
# Solved: the cryptographic mechanism used is AES-128
BLOCK_SIZE = 16
...
texto = unpad(decipher_aes.decrypt(ciphertext), BLOCK_SIZE).decode("utf-8", "ignore")

print(texto)

# hmacObj = HMAC.new(key, digestmod=SHA256)
# hmacObj.update(hash)
#
# hmacComp = HMAC.new(key, digestmod=SHA256)
# hmacComp.update(ciphertext)
#
# try:
#     hmacObj.hexverify(hmacObj)
#     print("The message '%s' is authentic" % texto)
# except ValueError:
...
cifrado_K_AT = cipher.encrypt(pad(M.encode('utf-8'), BLOCK_SIZE))
print("Cifrado K_AT: '%s'" % cifrado_K_AT)
...
cipher2 = DES.new(K_BT, DES.MODE_ECB)
cifrado_K_BT = cipher2.encrypt(pad(msg_K_AT, BLOCK_SIZE))
print("Cifrado K_BT: '%s'" % cifrado_K_BT)
...
decipher_des = DES.new(K_BT, DES.MODE_ECB)
decipher_des2 = DES.new(K_AT, DES.MODE_ECB)
descifrado_K_AT = unpad(decipher_des.decrypt(msg_K_BT), BLOCK_SIZE).decode("utf-8", "ignore")

print("Texto descifrado: '%s'" % descifrado_K_AT)
...
Cipher_M = engineRSACifrado.encrypt(M)

HMAC_M = HMAC.new(K_AB, M, digestmod=SHA256)
...
h = SHA256.new(msg_Cipher_M)
print(h.hexdigest())
verifier = pss.new(public)

if verifier.verify(h, msg_HMAC_M):
